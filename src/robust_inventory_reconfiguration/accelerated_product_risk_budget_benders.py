...
def _product_worker(connection, instance, gamma, product_indices):
    try:
        ...
        subproblems = {
            j: AcceleratedProductRiskSubproblem(instance, j, gamma)
            for j in product_indices
        }
        connection.send(("ready", perf_counter() - started))
        while True:
...
class _ExactParallelProductOracle:
    ...
    def evaluate(
        ...
        for (connection, _), pending in zip(self.connections, pending_by_worker):
            if pending:
                connection.send(("solve", [(j, vector) for j, _, vector in pending]))
        for (connection, _), pending in zip(self.connections, pending_by_worker):
            if not pending:
                continue
            status, payload = connection.recv()
        ...
        return [result for result in results if result is not None], perf_counter() - started
...
def solve_accelerated_prb_benders(
    ...
    *,
    ...
    workers = parallel_workers or min(instance.num_products, os.cpu_count() or 1)
    started = perf_counter()
    ...
    try:
        for iteration in range(1, max_iterations + 1):
            ...
            gamma_runtime += perf_counter() - gamma_started
        ...
        certified_results, _ = oracle.evaluate(best_solution.x, allow_cache=False)
        certified_values = [
            [worst.value for worst in result.worst_cases]
